...
vignette: >
  %\VignetteIndexEntry{ggpol Guide}
...
`geom_arcbar` plot arc bar diagrams that span 180 degrees. They also take an optional spacing argument `sep` that is entered as a total proportion of pi. This is best demonstrated with an example:
...
`geom_bartext` is a [Stackoverflow-inspired](https://stackoverflow.com/questions/50059193/jitter-text-labels-with-position-stack), experimental feature (and currently only available via the developer version) for unstacking overlapping text-labels in barcharts. If overlapping, `geom_bartext` will repel all groups of overlapping labels vertically or horizontally to make for a readable graph.
